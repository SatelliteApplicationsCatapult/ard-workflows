...
class DensifyGrid:
    ...
    def process(self, annotation_files, grid_pts=100):
        ...
        for f in annotation_files:
            ...
            if doc is not None:
                ...
                logging.info(f'Densifying geolocation grid in annotation file: {f}')
                ...
                dense_grid.update(
                    self.interpolateFields(doc, (gcps['latlon']['pixel'], gcps['latlon']['line']), (x_grid, y_grid)))

                # interpolate gcps onto dense grid
                geo_transform = gdal.GCPsToGeoTransform(gcps['mercator'])
                if logging.root.level == logging.DEBUG:
                    # this is kind of expensive. Don't calculate unless needed.
                    logging.debug(f'{os.path.basename(f)}: Mean sum of squares: {self.computeError(gcps["mercator"], geo_transform)} m ')
    ...
    def interpolateTiePoints(self, geo_t, pixel, line):
        ...
        count = 0
        logging.info(f"{num_samples} looping...")
    ...
    def reprojectTiePoints(self, gcps, projection, threads=4):
        ...
        logging.info(f"starting transform {threads}")
        # get task list
    ...
    def writeAnnotationFile(self, doc, dense_grid):
        ...
        while count < len(dense_grid['gcps']):
            ...
            obj['azimuthTime'] = datetime.fromtimestamp(dense_grid['azimuthTime'][count]).strftime(
                '%Y-%m-%dT%H:%M:%S.%f')
