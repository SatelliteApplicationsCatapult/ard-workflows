...
def reproject_wkt_3460_4326(wkt):
    """
    Reproject wkt geometry from wgs84 into Fiji local coordinate system
...
def reproWGS84GDF(in_gdf):
    ...
    TO DO:UPDATE PARAMS
    """
    in_gdf['repro'] = in_gdf['geometry'].apply(lambda x: reproject_wkt_3460_4326(str(x)))
    in_gdf['repro'] = in_gdf['repro'].apply(wkt.loads)
    in_gdf = gpd.GeoDataFrame(in_gdf, geometry='repro', crs=('epsg:4326'))
